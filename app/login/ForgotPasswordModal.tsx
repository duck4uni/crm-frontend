...
/** 6 individual OTP boxes with auto-focus */
    ...
    <div className="flex items-center justify-center gap-2">
      {Array.from({ length: OTP_LENGTH }).map((_, i) => (
        <input
          ...
          className="h-12 w-10 rounded-lg border border-gray-200 bg-gray-50 text-center text-lg font-bold text-indigo-700 caret-transparent focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/25 disabled:opacity-50 transition"
        />
      ))}
    </div>
  );
          ...
          {(["request", "verify", "reset"] as ForgotStep[]).map((s, i) => (
            <div key={s} className="flex items-center gap-1">
              <div className={`h-1.5 w-8 rounded-full transition-colors ${
                step === s ? "bg-indigo-600" : i < ["request","verify","reset"].indexOf(step) ? "bg-teal-400" : "bg-gray-200"
              }`} />
            </div>
          ))}
        </div>
