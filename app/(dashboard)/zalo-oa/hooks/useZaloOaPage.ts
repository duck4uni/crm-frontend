...
export function useZaloOaPage() {
    ...
    useEffect(() => {
        ...
        if (!fetchedConvIds.current.has(selectedConvId)) {
            ...
            if (cached.length > 0) {
                setMessagesByConversation((prev) => ({ ...prev, [selectedConvId]: cached }));
            }
        }
    ...
    const patchMessageInState = useCallback(
        (conversationId: string, messageId: string, patch: Partial<ZaloChatMessage>) => {
            setMessagesByConversation((prev) => {
                const current = prev[conversationId] || [];
    ...
    const performSend = useCallback(
        async (
            ...
            send: () => Promise<{ msgId: string; sentAt: string; attachmentUrl?: string; attachmentName?: string }>,
        ) => {
            // Insert optimistic
            ...
            updateConversationPreview(
                conversationId,
                optimistic.content || (optimistic.messageType === "image"
                    ? "[Hình ảnh]"
    ...
    const handleRetryMessage = useCallback(
        ...
        [selectedConvId, messagesByConversation, getOaForConversation, patchMessageInState],
    );
